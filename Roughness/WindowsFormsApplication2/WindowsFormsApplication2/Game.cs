...
namespace WindowsFormsApplication2 {
    ...
    static class Game {
        static public void star(Form1 cf) {
            Form1 curren_form1 = cf;
            curren_form1.game_map = new GameMap((int)cf.ClientSize.Width, (int)cf.ClientSize.Height);
            List<Bomb> bombs = new List<Bomb>();

            Player one = new Player(curren_form1, "P1", 1, 1, 50, 50, @"..\..\GameRes\Image1.png"); // TODO Перед сборпкой обязательно указать новые пути
            one.setKeysControl(Keys.Left, Keys.Up, Keys.Right, Keys.Down, Keys.Space);
            one.putBomb += ((int x, int y) => { bombs.Add(new Bomb(curren_form1, x, y)); });

            Player two = new Player(curren_form1, "P2", 400, 400, 50, 50, @"..\..\GameRes\Image1.png"); // TODO Перед сборпкой обязательно указать новые пути
            two.setKeysControl(Keys.A, Keys.W, Keys.D, Keys.S, Keys.F);
            two.putBomb += ((int x, int y) => { bombs.Add(new Bomb(curren_form1, x, y)); });

            List<Wall> walls = new List<Wall>();
            for (int x = 50; x <= 600; x += 100) for (int y = 50; y <= 500; y += 100) walls.Add(new Wall(curren_form1, "iw", x, y, 48, 48));
    ...
    abstract class GameObject {
        ...
        static readonly int k_max_x;
        static readonly int k_max_y;
        ...
        protected Direction curren_direction;
        private string m_id;
        protected Form1 curren_form;
        public Point m_location;


        //protected GameObject() : this(null, "", 0, 0, 0, 0) { }
        protected GameObject(Form1 cf, string id, int x, int y, int x_size, int y_size) : this(cf, id, x, y, x_size, y_size, null) { }
        public GameObject(Form1 cf, string id, int x, int y, int x_size, int y_size, string image_path) {
            curren_form = cf;
        ...
        static GameObject() {
            k_max_x = 0;// TODO потом исправить
            k_max_y = 0;
        }
    ...
    class Player : GameObject, IMortal, IAbleToMove {
        ...
        public Player(Form1 cf, string id, int x, int y, int x_size, int y_size, string image_path) : base(cf, id, x, y, x_size, y_size, image_path) {
            curren_form.KeyDown += new KeyEventHandler(ActionByKeyDown);
            curren_form.KeyUp += new KeyEventHandler(ActionByKeyUp);
            curren_form.game_timer.Tick += new EventHandler((object sender, EventArgs e) => { Move(curren_direction); });
            move_speed = 3;
        }
    ...
    class Wall : GameObject, IIsBarrier {
        public Wall(Form1 cf, string id, int x, int y, int x_size, int y_size) : base(cf, id, x, y, x_size, y_size) {
            setCollision();
        }
        ...
        public void setCollision() {
            ...
            for (int ix = x; ix < end_x; ix++)
                for (int iy = y; iy < end_y; iy++)
                    curren_form.game_map.CollisionsMap[ix][iy] = true;
        }
    }

    class Bomb : GameObject, IIsBarrier {
        public Bomb(Form1 cf, int x, int y) : base(cf, "b", x, y, 40, 40, @"..\..\GameRes\Image3.png") {
            setCollision();
        }
        ...
        ~Bomb() { // Тут очень важно иметь деструптор
